...
/* Adrian Widget — portable floating assistant
 *
...
(function (global) {
  ...
  SomaGuide.prototype._build = function () {
    ...
    // Instead of building a complex DOM, we use soma-assist-core
    this.chip = SomaAssistCore.createAssistChip({
      appId: 'soma-guide',
      title: name,
      avatar: avatar,
      onUserMessage: function(text) {
        if (this._onUserMsg) this._onUserMsg(text);
      }.bind(this),
      mount: document.body
    });

    // Provide some wrapper elements so existing code doesn't crash on null
    var el = document.createElement('div');
    el.id = 'soma-guide';
    el.className = 'sg sg--min';
    this.el = el;
    
    // The chip handles open/close itself, but we can map methods
    this.ui = {
      body: { appendChild: function(){} }, // dummy
  ...
  SomaGuide.prototype._open = function () {
    this.chip.open();
  };
