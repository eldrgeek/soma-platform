...
const [source, css, scriptRuntime, feedback, heartbeat, capture] = await Promise.all([
  ...
  readFile(join(root, 'src/soma-assist-capture.js'), 'utf8')
]);
...
await Promise.all([
  ...
  writeFile(join(root, 'dist/soma-assist-capture.js'), capture)
]);
console.log('Built assist UI, script runtime, feedback, heartbeat, and capture artifacts');
